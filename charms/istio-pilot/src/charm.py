...
class Operator(CharmBase):
    ...
    def remove(self, event):
        ...
        for resource in self._custom_resources.values():
    ...
    def handle_default_gateway(self, event):
        ...
        self._resource_handler.delete_existing_resource_objects(
            resource=self._custom_resources['gateway'],
            labels={
    ...
    def handle_ingress_auth(self, event):
        ...
        self._resource_handler.delete_existing_resource_objects(
            self._custom_resources['envoy_filter'], namespace=self.model.name
        )
        self._resource_handler.apply_manifest(auth_filters, namespace=self.model.name)

    @property
    def _custom_resources(self) -> dict:
        """Returns a dictionary with all custom resources names as keys and classes as values."""
        ...
        envoy_filter_resource = self._resource_handler.create_ns_resource(
            filename='auth_filter.yaml.j2', context=context
        )
        virtual_service_resource = self._resource_handler.create_ns_resource(
            filename='virtual_service.yaml.j2', context=context
        )
        gateway_resource = self._resource_handler.create_ns_resource(
            filename='gateway.yaml.j2', context=context
        )
        custom_resources = {
            'envoy_filter': envoy_filter_resource,
            'virtual_service': virtual_service_resource,
    ...
    def _resource_handler(self):
        """Returns an instanciated ResourceHandler."""
        config = {"app_name": self.app.name, "model_name": self.model.name}
        return ResourceHandler(config)
